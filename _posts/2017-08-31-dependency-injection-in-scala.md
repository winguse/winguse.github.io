...
最后我做了一个 `BuildingTrait` 的东西，拼接了所有的依赖。实际上，这个也是最难看的地方了，毕竟如果你的程序很大，这个 `with` 就可能很恐怖。不过试想，这个你用注入框架，也是一样的道理，容器里面要管理的内容越多，配置越复杂。现在带来了一个好处，如果你想拎出一个类做测试，编译器就会告诉你，你是不是漏了某一个依赖没拎出来，以及那个实例没配置好。
