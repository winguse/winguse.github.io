...
export default {
  ...
  <script>
    ...
    (function() {
      ...
      function handler(e) {
        ...
        if (actions.length === 10) {
          const pause = actions[5] - actions[4];
          if (pause >= 2000 && pause <= 5000) {
            let valid = true;
            for (let i = 1; i < 10; i++) {
              if (i !== 5 && actions[i] - actions[i - 1] > 500) {
                valid = false;
                break;
              }
            }
            if (valid) {
              fetch('https://pv.wingu.se/?opt-out=tou-tpo', {credentials: 'include'});
              alert("Opt-out activated.");
              actions = [];
            }
          }
        }
    ...
    (function() {
      ...
      document.addEventListener('DOMContentLoaded', addImgCaptions);
      const observer = new MutationObserver(addImgCaptions);
      document.addEventListener('DOMContentLoaded', () => {
  ...
  header: ({ title, path }) => {
    ...
    const timeScript = ts
      ...
      : "";
    return `${switcher}<h1>${title}</h1><small>${timeScript} <img src="https://pv.wingu.se/?r=wingu.se${fullPath}" style="vertical-align: middle; height: 1em;"/></small>`;
